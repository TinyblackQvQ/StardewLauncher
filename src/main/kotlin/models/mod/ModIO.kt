...
class ModIO {
    ...
    constructor(targetMod: Mod, modResourceDirectory: String = "./mods") {
         ...
         * 2. 使用 .mod.manifest.uniqueID-mod.manifest.version 作为目录名进行读取，对于该目录被禁用的情况（首部存在一个"."将禁用该mod）
         * 3. 强制搜索整个目录来寻找目标mod
         * */
        ...
        if (!Mod.isDirectoryAValidMod(modDirectory))
            for (file in ModResourceView.getAllModDirectories(modResourceDirectory)) {
